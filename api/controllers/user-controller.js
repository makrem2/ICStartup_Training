...
const PrivateKey="this is privekeyqsdqsdqsghdhsqdjksqdhjsqdhsqdgsqhgdhg!!:qs;!é&sqhgdhgsqdsq"
        ...
        db.User.findOne({where:{email:email}}).then(user=>{
            if (!user){
             ...
             bcrypt.compare(password,user.password).then(same=>{
                 if(same){
                     let token = jwt.sign({id:user.id,username:user.username,role:'Admin'},PrivateKey,{
                     expiresIn:"24h"
                    })
